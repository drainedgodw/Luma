...
# It keeps a compatible Node.js toolchain inside .luma/ and never changes the
# user's system Node installation or shell configuration.
...
Commands:
  ...
  setup        Prepare the local Node toolchain and project dependencies
  ...
  toolchain    Download/check only the local Node toolchain
  clean        Remove generated project files, but keep the downloaded Node toolchain
...
Options:
  ...
  --install-system-deps   Install missing Git/Python/compiler packages via the OS package manager

Environment overrides:
  LUMA_USE_SYSTEM_NODE=1  Use an already-installed compatible Node instead of .luma/toolchain
  LUMA_NODE_VERSION=22    Override the version/channel read from .nvmrc
  LUMA_NODE_DIST_URL=...  Use a Node.js distribution mirror
  LUMA_STATE_DIR=...      Store the private toolchain/cache elsewhere
...
PY
    ...
    die 'Need curl, wget, or Python 3 to download the private Node toolchain.'
  fi
}
...
system_dependency_command() {
  if command -v pacman >/dev/null 2>&1; then printf 'sudo pacman -S --needed git base-devel python\n'
  elif command -v apt-get >/dev/null 2>&1; then printf 'sudo apt-get update && sudo apt-get install -y git build-essential python3\n'
  elif command -v dnf >/dev/null 2>&1; then printf 'sudo dnf install -y git gcc-c++ make python3\n'
  elif command -v zypper >/dev/null 2>&1; then printf 'sudo zypper --non-interactive install git gcc-c++ make python3\n'
  elif command -v apk >/dev/null 2>&1; then printf 'sudo apk add git build-base python3\n'
  else printf 'Install Git, Python 3, make and a C/C++ compiler with your package manager.\n'; fi
}

install_missing_system_deps() {
  if command -v pacman >/dev/null 2>&1; then root_command pacman -S --needed --noconfirm git base-devel python
  elif command -v apt-get >/dev/null 2>&1; then root_command apt-get update; root_command apt-get install -y git build-essential python3
  elif command -v dnf >/dev/null 2>&1; then root_command dnf install -y git gcc-c++ make python3
  elif command -v zypper >/dev/null 2>&1; then root_command zypper --non-interactive install git gcc-c++ make python3
  elif command -v apk >/dev/null 2>&1; then root_command apk add git build-base python3
...
expected_install_stamp() {
  local lock_hash node_version npm_version
  ...
  printf 'schema=%s\nlock=%s\nnode=%s\nnpm=%s\nplatform=%s-%s\n' "$BOOTSTRAP_SCHEMA" "$lock_hash" "$node_version" "$npm_version" "$PLATFORM" "$ARCH"
}
...
setup_project() { detect_platform; ensure_system_deps; ensure_node; install_dependencies; }
...
main() {
  ...
  case "$COMMAND" in
    help) usage ;;
    clean) rm -rf -- "$ROOT_DIR/node_modules" "$ROOT_DIR/out" "$ROOT_DIR/dist" "$INSTALL_STAMP"; log 'Removed dependencies and generated builds; kept the private Node toolchain.' ;;
    purge) rm -rf -- "$ROOT_DIR/node_modules" "$ROOT_DIR/out" "$ROOT_DIR/dist" "$STATE_DIR"; log 'Removed dependencies, generated builds and the private Node toolchain.' ;;
    toolchain) detect_platform; ensure_node; log "Node: $($NODE_BIN --version); npm: $($NPM_BIN --version)" ;;
    setup) setup_project ;;
