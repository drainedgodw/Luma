...
type View = 'editor' | 'graph' | 'orbit' | 'changes' | 'languages' | 'settings' | 'rescue';
...
function Shell() {
  ...
  const commands: Command[] = useMemo(() => {
    ...
    return [
      ...
      cmd('view-orbit', 'Go to Orbit graph', 'View', () => setView('orbit')),
    ...
    <div className="relative flex h-full w-full flex-col">
      <header className="glass mx-3 mt-3 flex h-12 items-center gap-3 px-4" style={{ WebkitAppRegion: 'drag' } as never}>
        ...
        {status && status.state !== 'branch' && status.state !== 'detached' && (
          <span className="rounded-full border border-amber/50 bg-amber/10 px-2 py-0.5 text-[11px] text-amber">{status.state} in progress</span>
        )}
      ...
      <div className="flex min-h-0 flex-1 flex-col gap-2 p-3">
        <div className="flex min-h-0 flex-1 gap-3">
          <nav className="glass flex w-14 flex-col items-center gap-1.5 py-3">
            ...
            <DockBtn active={pinned} onClick={() => update({ explorer: pinned ? 'auto' : 'pinned' })} label={pinned ? 'Hide sidebar (slides out on hover)' : 'Show sidebar permanently'} text="Panel" icon={<Icon name="panel" />} />
            ...
            <DockBtn active={view === 'orbit'} onClick={() => setView('orbit')} label="Orbit commit graph" text="Orbit" icon={<Icon name="graph" />} />
          ...
          <main className="flex min-w-0 flex-1 flex-col gap-2">
            ...
            <div className={`min-h-0 flex-1 ${showTerminal ? 'flex flex-col gap-2' : 'flex flex-col'}`}>
              <div className="min-h-0 flex-1">
                ...
                ) : view === 'graph' ? <GraphView onRebase={(b) => setRebaseTarget(b)} /> : view === 'orbit' ? <OrbitView /> : view === 'changes' ? <ChangesView onOpenFile={openFile} /> : view === 'languages' ? <StoreView /> : view === 'settings' ? <SettingsView /> : view === 'rescue' ? <RescueView /> : <EditorWorkspace />}
              </div>
              {showTerminal && <div className="h-[38%] min-h-[160px]"><TerminalPanel onClose={() => setShowTerminal(false)} /></div>}
            </div>
          </main>
        </div>
      </div>
...
function WinBtn({ title, onClick, children, danger }: { title: string; onClick: () => void; children: React.ReactNode; danger?: boolean }) {
  return (
    <button title={title} onClick={onClick} className={`flex h-8 w-9 items-center justify-center rounded-lg text-[11px] transition-colors ${danger ? 'text-white/60 hover:bg-rose hover:text-white' : 'text-white/60 hover:bg-white/10 hover:text-white'}`}>
      {children}
    </button>
  );
}
...
function DockBtn({ active, onClick, label, icon, text, badge }: { active: boolean; onClick: () => void; label: string; icon: React.ReactNode; text?: string; badge?: string | null }) {
  return (
    <button title={label} onClick={onClick} className={`relative flex h-[52px] w-11 flex-col items-center justify-center gap-0.5 rounded-xl border transition-all duration-200 ${active ? 'border-lilac/50 bg-lilac/15 text-lilac shadow-[0_0_16px_rgba(196,181,253,0.25)]' : 'border-transparent text-white/45 hover:border-white/10 hover:bg-white/5 hover:text-white/80'}`}>
      {icon}
      {text && <span className="text-[8px] tracking-wide opacity-70">{text}</span>}
      {badge && <span className="absolute -right-1 -top-1 rounded-full bg-teal px-1.5 text-[10px] font-bold text-void">{badge}</span>}
    </button>
  );
}
...
function Welcome() {
  ...
  return (
    <div className="flex h-full flex-col items-center justify-center gap-8">
      ...
      <button className="btn btn-primary px-8 py-3 text-sm" onClick={() => openRepo()}>Open a repository</button>
      {recent.length > 0 && (
        <div className="glass anim-in w-[420px] p-3">
          ...
          {recent.map((r) => (
            <button key={r} className="block w-full truncate rounded-lg px-3 py-2 text-left font-mono text-xs text-white/70 hover:bg-white/5" onClick={() => openRepo(r)}>{r}</button>
          ))}
        </div>
      )}
    </div>
