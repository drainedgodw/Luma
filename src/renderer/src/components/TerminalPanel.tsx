...
const THEMES = {
  cosmos: { bg: 'transparent', fg: '#e6e6f0', cursor: '#c4b5fd', selection: 'rgba(139,92,246,.35)' },
  liquid: { bg: 'transparent', fg: '#f2f4fa', cursor: '#a5f3fc', selection: 'rgba(165,243,252,.30)' },
} as const;
...
export default function TerminalPanel({ onClose }: { onClose: () => void }) {
  ...
  useEffect(() => {
    ...
    void api.intelInvoke('trustStatus').then((result) => {
      ...
      if (!result.ok) setTrustError(result.error?.message ?? 'Could not read workspace trust status.');
    });
    return () => { active = false; };
  }, [repo]);
  ...
  async function trustRepository() {
    if (!window.confirm('Trust this repository? Terminal commands and local tasks can execute code from it.')) return;
    setTrustError('');
  ...
  return (
    <div className="term-panel flex h-full min-h-0 flex-col overflow-hidden rounded-[14px] border">
      ...
      <div className="flex items-center gap-2 border-b border-white/8 px-3 py-1.5">
        ...
        <button className="text-[11px] text-white/40 hover:text-white" onClick={onClose}>✕</button>
      </div>
      {trust === 'trusted' ? (
        ...
        <div className="flex min-h-0 flex-1 items-center justify-center p-6">
          <div className="max-w-lg text-center">
            <div className="text-sm font-semibold text-amber">Terminal locked for this repository</div>
            <p className="mt-2 text-xs leading-5 text-white/45">
              Luma blocks shell execution until you explicitly trust the repository. This prevents an unfamiliar project from running local commands without your approval.
            </p>
            {trustError && <div className="mt-2 text-xs text-rose">{trustError}</div>}
            <button className="btn btn-primary mt-4" disabled={trust === 'checking'} onClick={() => void trustRepository()}>
