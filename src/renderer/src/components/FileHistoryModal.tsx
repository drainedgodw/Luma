...
export default function FileHistoryModal({ path, onRestore, onClose }: { path: string; onRestore: (content: string) => void; onClose: () => void }) {
  ...
  return (
    <div className="fixed inset-0 z-[60] flex items-center justify-center bg-black/50 backdrop-blur-sm" onMouseDown={onClose}>
      <div className="glass anim-in flex h-[70vh] w-[820px] flex-col overflow-hidden" onMouseDown={(e) => e.stopPropagation()}>
        ...
        <div className="flex min-h-0 flex-1">
          <div className="w-52 shrink-0 overflow-y-auto border-r border-white/8 p-2">
            ...
            {snaps?.length === 0 && <div className="p-2 text-xs text-white/40">No snapshots yet — they are taken on every save.</div>}
            {snaps?.map((ts, i) => (
              <button
                key={ts}
                onClick={() => show(ts)}
          ...
          <div className="min-w-0 flex-1 overflow-auto p-4">
            {!preview && <div className="text-xs text-white/40">Pick a snapshot to preview it.</div>}
            {preview && (
              <>
                <pre className="mb-3 max-h-[80%] overflow-auto rounded-xl border border-white/10 p-3 font-mono text-[12px] leading-relaxed text-white/70" style={{ userSelect: 'text' }}>
