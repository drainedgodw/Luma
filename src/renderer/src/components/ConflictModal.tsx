...
export default function ConflictModal({ file, onClose, onResolved }: { file: ConflictFile; onClose: () => void; onResolved: () => void }) {
        ...
        <div className="min-h-0 flex-1 overflow-y-auto p-4">
          {file.regions.length === 0 && (
            <div className="mb-3 text-xs text-white/50">
              No conflict markers found — the file may already be resolved, or markers use a custom format.
            </div>
          )}
          {file.regions.map((r, idx) => {
            const srcStart = findRegions(file.ours.split('\n'), 'ours')[idx]?.startLine ?? r.startLine;
            ...
            return (
              <div key={idx} className="mb-4 overflow-hidden rounded-xl border border-white/10">
                <div className="flex items-center gap-2 border-b border-white/10 bg-white/4 px-3 py-2">
                  <span className="text-[11px] text-white/50">Conflict #{idx + 1} · line {r.startLine + 1}</span>
                ...
                <div className="grid grid-cols-2 font-mono text-[12px]">
                  <div className="dif-del">
                    <div className="bg-rose/15 px-3 py-1 text-[10px] uppercase tracking-wide text-rose">Ours</div>
                    {r.ours.slice(0, 30).map((l, i) => <div key={i} className="whitespace-pre px-3 dif-del-text">{l || ' '}</div>)}
                  </div>
                  <div className="dif-add">
                    <div className="bg-teal/15 px-3 py-1 text-[10px] uppercase tracking-wide text-teal">Theirs</div>
                    {r.theirs.slice(0, 30).map((l, i) => <div key={i} className="whitespace-pre px-3 dif-add-text">{l || ' '}</div>)}
                  </div>
                </div>
              </div>
            );
          ...
          {file.base && (
            <details className="mt-2">
              <summary className="cursor-pointer text-[11px] text-white/40">Show common ancestor</summary>
              <pre className="mt-2 max-h-48 overflow-auto rounded-lg bg-black/30 p-3 font-mono text-[11px] text-white/50">{file.base}</pre>
            </details>
          )}
        </div>

        <div className="flex items-center gap-2 border-t border-white/10 px-4 py-3">
          <span className="text-[11px] text-white/40">{remaining > 0 ? `${remaining} region(s) default to "ours"` : 'All regions decided'}</span>
          <div className="flex-1" />
          <button className="btn text-xs" onClick={() => resolve('ours')}>All ours</button>
          <button className="btn text-xs" onClick={() => resolve('theirs')}>All theirs</button>
          <button className="btn btn-primary text-xs" onClick={() => resolve()}>Resolve & stage</button>
        </div>
      </div>
    </div>
  );
