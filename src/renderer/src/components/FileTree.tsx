...
export default function FileTree() {
  ...
  return (
    <>
      {/* hover edge — the panel wakes when the cursor touches the left border */}
      <div
        className="fixed left-0 top-0 z-40 h-full w-2"
        onMouseEnter={() => setVisible(true)}
      />
      <div
        className={`glass fixed left-3 top-[68px] bottom-3 z-40 flex w-60 flex-col overflow-hidden transition-all duration-300 ${
          shown ? 'translate-x-0 opacity-100' : 'pointer-events-none -translate-x-[270px] opacity-0'
        }`}
