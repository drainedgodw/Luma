...
interface LumaApi {
  ...
  githubStatus(): Promise<GitResult<{ connected: boolean; account?: { login: string; name?: string; avatarUrl?: string }; deviceFlowAvailable: boolean }>>;
  githubSaveToken(t: string): Promise<GitResult<{ login: string; name?: string; avatarUrl?: string }>>;
  githubLogout(): Promise<GitResult<null>>;
  githubRepos(q?: string): Promise<GitResult<Array<{ id: number; name: string; fullName: string; description?: string; private: boolean; updatedAt: string; defaultBranch: string; httpsUrl: string; sshUrl: string; owner: string }>>>;
