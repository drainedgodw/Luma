...
function createWindow() {
  ...
  if (process.env.LUMA_REPO) {
    ...
    if (process.env.LUMA_OPEN_FILE) {
      ...
      win.webContents.executeJavaScript(`setTimeout(() => window.dispatchEvent(new CustomEvent('luma:open-file', { detail: ${file} })), 800)`).catch(() => {});
    }
  }
  if (process.env.LUMA_THEME) {
    const theme = process.env.LUMA_THEME;
    win.webContents.on('did-finish-load', () => { win?.webContents.executeJavaScript(`window.dispatchEvent(new CustomEvent('luma:theme', { detail: ${JSON.stringify(theme)} }))`).catch(() => {}); });
  }
  if (process.env.LUMA_SHOT) {
    setTimeout(async () => { const image = await win!.webContents.capturePage(); const { writeFile } = await import('node:fs/promises'); await writeFile(process.env.LUMA_SHOT!, image.toPNG()); app.quit(); }, 4000);
  }
