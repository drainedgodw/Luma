...
type DirectoryResult = { ok: true; data: string } | { ok: false; error: { message: string; stderr: string } };

const MAX_RECENT = 12;
const recentFile = () => join(app.getPath('userData'), 'recent.json');
const failed = (message: string): DirectoryResult => ({ ok: false, error: { message, stderr: '' } });
...
async function recentDirectories(): Promise<string[]> {
  ...
  if (valid.length !== stored.length || valid.some((directory, index) => directory !== stored[index])) await writeStored(valid);
  return valid;
}
...
async function openDirectory(getWindow: () => BrowserWindow | null, directory: string): Promise<DirectoryResult> {
...
export function registerRecentReposIpc(getWindow: () => BrowserWindow | null): void {
  ipcMain.handle('repo:directory:open', async (): Promise<DirectoryResult> => {
    const window = getWindow();
    if (!window) return failed('Luma window is not ready');
    const result = await dialog.showOpenDialog(window, { title: 'Open project directory', properties: ['openDirectory'] });
    if (result.canceled || !result.filePaths.length) return failed('canceled');
    return openDirectory(getWindow, result.filePaths[0]);
  });
  ipcMain.handle('repo:directory:openPath', async (_event, directory: string): Promise<DirectoryResult> => {
