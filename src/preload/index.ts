...
contextBridge.exposeInMainWorld('luma', {
  ...
  gitInvoke: (channel: string, ...args: unknown[]) => channel === 'commit'
    ...
    : channel === 'rewindSoft'
      ? ipcRenderer.invoke('intel:git:rewind', 'soft', ...args)
      : channel === 'rewindHard'
        ? ipcRenderer.invoke('intel:git:rewind', 'hard', ...args)
        : ipcRenderer.invoke(SECURE_GIT_CHANNELS.has(channel) ? `github:git:${channel}` : `git:${channel}`, ...args),
  ...
  onCommand: (cb: (e: { id: number; command: string; at: number }) => void) => ipcRenderer.on('git:command', (_e, x) => cb(x)),
  ...
  termOnData: (id: string, cb: (d: string) => void) => ipcRenderer.on(`term:data:${id}`, (_e, d) => cb(d)),
